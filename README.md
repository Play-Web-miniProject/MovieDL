...
***다른 방법 고민***
1. 전국 관객수의 뒷자리를 자르고 회귀모델을 가져가자 
=> 의미가 있을까??? 

2. 전국 관객수를 그룹화해서 분류 모델로 변경 
=> 라벨링은 어떻게 할 거야??? 
임의대로 라벨링하면은 좋은 학습 못해
각 라벨에 적당한 수가 들어갈 수 있도록 고민해보자

2-1) 랭킹을 매겨서 줄을 세워서 극 값을 기준으로 어쩌구...
2-2) 동일한 간격으로 그룹 (그룹 1,2,3,4..)
2-3) 전국관객수 뒷자리를 자르고 회귀모델로도 가져갈 수 있어(*회귀*) (위에랑 뭐가 달라...?)
2-4) 각각의 라벨의 최솟값을 뒷자리도 잘라  (예지언니 뭐라고..?)

# 딥러닝을 활용한 영화 투자 조건 분석 

### 팀 소개
|구분|이름|내용|
|--|--|--|
|팀명||5OS(오링스)|
|팀원||안태현, 오성진, 오수연, 유정현, 이예지|
|역할|안태현|데이터 수집 및 기초 전처리, 딥러닝 모델 선정 및 학습, 파라미터 조정|
||오성진|데이터 컬럼별(배급사 등) 전처리, 점수화, 딥러닝 모델 선정 및 학습, 파라미터 조정|
||오수연|데이터 컬럼별(개봉일 ▶ 연도, 계절) 전처리, 점수화, 딥러닝 모델 파라미터 조정|
||유정현|데이터 컬럼별() 전처리, 점수화, 딥러닝 모델 파라미터 조정|
||이예지|추가 분석용 웹 크롤링, 크롤링 데이터 전처리|

### data 설명
|순번|파일명|설명|
|--|--|--|
|1|/data/movie_data/KOBIS_개봉일람_ 2023-03-20.csv|원본 데이터 파일|
|2|/1_preprocessing/preprocessing.csv|누락 제외, 점수화, 라벨링 컬럼 추가|
|3|/2_resizing/resizing.csv|스크린수 50 초과 , 전국 관객수 1000명 초과 1100만 미만|
|4|/data/movie_data/concat.csv|네이버 평점과 배우 정보|
|5|/2_resizing/movie_resize_merged.csv|resizing.csv 데이터에 평점&배우 정보 합친 것|
|6|/2_resizing/movie_resize_ranking.csv|top300 데이터|
|7|/2_resizing/movie_final.csv|전처리 완료 데이터|

### 제출 파일( 발표자료 및 코드, 데이터 포함)  
<br>
<br>

---

### 2023.04.03. 회의록

1. Top300 문제: '전국 관객수' 적은 데이터(회귀용)에서 top300 데이터를 그대로 써도 되나?   
> 점수화하는 거기 때문에 써볼만 하다.   

2. Top300에도 1100만 이상 데이터 포함해서 다시 만들기   

3. '연도', '코로나' 컬럼 추가한 resize 파일 -> All, 분류, 회귀로 나뉜다.   

4. '전국 관객수' 등의 스케일링은 2023 데이터 자르기 전(!!!!)에 실시   

### 선생님 조언 후 정리 (23.03.31.)   
1. 지금껏 했던 *회귀 모델*의 문제: test 세트로 predict할 시, 관객수가 큰 데이터에 대해서 loss가 크다.   
> 그렇다면, 관객수 구간 별로 라벨링(= 분류 클래스)을 해서 *분류 모델*로 바꾸면 어떨까?   
   
2. 관객수 구간별로 나눌 때의 문제: 영화가 많이 몰려있는 '적은 관객 수' 구간은 간격이 촘촘한 데 비해, '많은 관객 수' 구간은 간격이 너무 넓다.   
> 구간 예측이 잘 되더라도 현실적인 문제 = '200만에서 1000만 사이의 관객이 예측된다'고 결론을 내리는 것이 의미가 있나?   

3. 선생님께서 도와주셔서 내린 결론: 회귀 모델과 분류 모델을 모두 가져가보자.   
> (1) 회귀 모델: 관객 수 예측 - 관객 수가 많은 데이터를 잘라내서 loss를 줄여보자.   
> (2) 분류 모델: 관객 수 구간 예측 - 관객 수가 적은 데이터를 잘라내서 큰 관객 수 구간을 조금 더 촘촘하게 잘라보자.   
> - 특정 구간 이상이면 흥행을 예측할 수 있다고 말할 수 있다. : (참고) 이건 모델한테 시킬 건 아니고 그냥 우리가 해석하는 거.   
   
(주의) 한꺼번에 다 가져가려고 욕심부리지 말자. 데이터 좀 작게 가져가도 괜찮다. 일단 모델을 만들고 넣고 돌려보는 것을 목표로 하자.

4. 해야할 일   

(1) movie_resize 이전 데이터를 가지고 두 가지 데이터셋 만들기   
(1-1) 회귀용 데이터: 관객 수 많은 데이터 버리기    
(1-2) 분류용 데이터: 관객 수 적은 데이터 버리기 (1100만 이상 데이터도 있어야 함!! ex. 명량)   

(2) (1)을 위해서 데이터 버리는 기준을 '전국 관객수' 기준으로, **우리가 정해야 함**   
(2-1) 회귀용 데이터: (예를 들어, 상위 30퍼 버리기)   
(2-2) 분류용 데이터: (예를 들어, 하위 60퍼 버리기 = 약 100만 이상) (각 클래스 별로 데이터가 비슷하게 들어가도록 나누자)   

(3) 회귀, 분류 모델 학습 시, '연도'를 intger로 추가해주기   

(4) 회귀, 분류 모델 학습 시, '코로나'(기간)을 0, 1 데이터로 추가해주기 (20~22년 = 1, 그 외 = 0)   

---

### 2023.03.31. 회의록

딥러닝 모델이 관객수를 정확히 맞추는 것은 불가능하지 않을까?

***다른 방법 고민***
<br> 1. 전국 관객수의 뒷자리를 자르고 회귀모델을 가져가자 
<br> => 의미가 있을까??? 
<br>
<br> 2. 전국 관객수를 그룹화해서 분류 모델로 변경 
<br> => 라벨링은 어떻게 할 거야??? 
<br> 임의대로 라벨링하면은 좋은 학습 못해
<br> 각 라벨에 적당한 수가 들어갈 수 있도록 고민해보자
<br> 
<br> 2-1) 랭킹을 매겨서 줄을 세워서 극 값을 기준으로 어쩌구...
<br> 2-2) 동일한 간격으로 그룹 (그룹 1,2,3,4..)
<br> 2-3) 전국관객수 뒷자리를 자르고 회귀모델로도 가져갈 수 있어(*회귀*) (위에랑 뭐가 달라...?)
<br> 2-4) 각각의 라벨의 최솟값을 뒷자리도 잘라  (예지언니 뭐라고..?)
<br> 

**결론**
<br> 총 3,104의 데이터를 310개씩 들어갈 수 있도록 구간을 나눠   
해야할 것: '전국 관객수' 기준으로 moive_resize2 데이터를 sort한 후 3104개 데이터를 10개 라벨로 나누자.   
각 라벨의 관객수 범위는 각 라벨의 '최솟값~최댓값'   
학습시킬 때 컬럼에 '연도'-'총 관객수'-'계절' 넣어주세여   
<br>

1. 연도 int로 바꾸고
2. 코로나 컬럼 추가하고(20~22년만 1, 나머지 0)
3. '연도', '코로나', '총 관객수' 조합해서 예측할 수 있도록, 학습에 같이 넣어주고
4. target 데이터를 라벨로 조정하고(예지가 할 일....)
5. 분류 모델로 만들기

---

### 2023.03.27. 회의록

1. 데이터 스케일링 : (해당 영화 관객수) / (해당 연도 전국 관객수) ▶ 종속 변수
2. 변수를 나눠서 모델 학습 시켜보기(장르, 계절, 감독, 배우, 배급사...)
3. 범주형 데이터를 어떻게 하느냐아ㅏ
<br>

---

### 2023.03.24. 회의록  

#### check 
<br> (수연)movieSeason_2003_2023.csv 파일 년도 및 계절 컬럼 추가 완료
<br> (성진)미니 프로젝트 영화 딥러닝.ipynb
<br> (태현)top200acd.csv => top200 랭킹 파일 => 2개 비어 => 202번째까지 받아와 200위 맞춤
<br> (태현)movie_eda_ver03.ipynb => 감독, 배급사 점수 처리를 진행함
<br> * 배급사 중 'CJ ENM'은 '(주)씨제이이엔엠'으로 변경
<br>
<br>
#### TO DO
<br> **감독, 배우, 배급사 -> 점수 가중치 고민 필요**
<br> - 평균을 내자 / 표준화 / 맥스 값으로 나눠 ...
<br> - 점수화 내는 과정에서 연도별 데이터를 가져가자(코로나 전/후 달라)
<br> - 연도별 전체 관람수 컬럼을 넣자 - 예지
<br> - 영화 개봉 연도의 전체 관람객수를 분모로 해서 각 영화의 점수를 만들자 - 성진
<br> 
<br> **흥행 여부(이진분류) vs 관객수 예측**
<br> 
<br> **딥러닝 정확도, 손실 맞추는 과정 필요**
<br> => 훈련데이터에서는 어느 정도 정확도 나오는데 테스트 데이터 정확도 너무 떨어짐
<br> - 머신러닝과 딥러닝으로 나눠서 파라미터 값이나 조정 필요
<br>

---

### 2023.03.23. 회의록  

#### check 
<br> resize된 약 4000개의 영화 리스트"는 2003 ~ 2023년 데이터
<br> 별도의 서울시 스크린수 데이터가 없어서 
<br> -> resize 조건: 전국 관객수 10000명이상 조정해서
<br> 새로운 "movie_resized2.csv" 생성 완료 (태현)
<br>
<br> (10000명 미만, 독립 및 예술(돈이 목적이 아닌경우)영화의 경우, 스크린수와 관객수 데이터가 의미 없다고 생각해서 이상데이터라 생각해 제외함. 4642
<br> 
<br> "kobis 역대 200" 데이터를 새롭게 하려 했으나 기존의 개봉일람 데이터에서 전국 관객수 기준으로 "top 200"을 이용
<br> 
<br> 감독, 배우, 배급사 -> "top200"리스트 순위를 만들고 20씩 구간별로 1~10점 
<br> 배우, 배급사, 감독 top200 점수
<br> 1 ~ 20 10점
<br> 21 ~ 40 9점
<br> 41 ~ 60 8
<br> 61 ~ 80 7
<br> 81 ~ 100 6
<br> 101 ~ 120 5
<br> 121 ~ 140 4
<br> 141 ~ 160 3
<br> 161 ~ 180 2
<br> 181 ~ 200 1 
<br> 
<br> "순번 top200영화 배우 배급사 감독"      <- top200acd.csv
<br> -> 배우는 아직 데이터가 없으므로   배급사랑 감독 점수화 
<br> 
<br> resized 된걸로 다시 크롤링 
<br>
 
---

### 2023.03.22.수 회의록
<br> 1번 파일에서 전체 19517행에서 null값 적당히 버리고 
<br> 영화명 19161
<br> 감독 7520(7791)
<br> 배급사 814(1327)
<br> 개봉일
<br> 영화유형 1
<br> 영화형태: 4 
<br>     장편19433 옴니버스79 단편4 기타1
<br> 국적 8(72)
<br>     한국6331 일본5413 미국3888 프랑스644 영국599 중국456 독일304
<br>     캐나다227 홍콩205 스페인187.. : 나머지 기타
<br> 전국스크린수 
<br> 전국 매출액 
<br> 전국 관객수 
<br> 장르 21
<br>     ['사극', '코미디', '판타지', '드라마', '액션', '애니메이션', 'SF', '범죄', '어드벤처', '전쟁','미스터리', 
<br>     '서부극(웨스턴)', '멜로/로맨스', '스릴러', '다큐멘터리', '공포(호러)', '가족','뮤지컬', '기타', '공연', '성인물(에로)']
<br> 등급 4(17) 
<br>     청소년관람불가 15세관람가 12세관람가 전체관람가
<br> 영화구분 1(2) 
<br>     일반영화 독립/예술영화
<br> 
<br> 
<br> 
#### 이용하려는 주요 독립변수
<br> 장르, 등급, 시즌(개봉일), 국적
<br>

<br> 감독, 배급사, 배우 => 범주형 데이터를 수치형 데이터로 변환하는 과정
<br> -   감독, 배급사 -> (영어이름 중복확인) -> 점수화
<br> -   배우, 평점 -> 네이버 크롤링 -> 점수화 
<br> 
<br> 
#### SUMMARY
<br> 우리는 이번 프로젝트에서 
<br> <랜덤포레스트(지도머신러닝 알고리즘)>를 통해서 데이터 전처리 후 주요변수 선택 예정 
<br> <다층 퍼셉트론 Multi-Layer Perceptron(cnn계열)>을 딥러닝 모델이 이용할 예정 
<br> 
<br> 독립변수 중 일부 점수화 
<br> 감독, 배우, 배급사 -> top200에서 구간별로 1~10점 
<br> 평점 -> 숫자 그대로 이용
<br> 
<br> '흥행여부'를 결정할 '전국 관객수' 기준 생각해보기  
<br>  
<br> 
#### ISSUE
<br> "resize된 약 4000개의 영화 리스트"는 2003 ~ 2022년 데이터 (resize 조건: 전국 관객수 1000 ~ 11000000명, 전국 스트린수 50)
<br> "top 200 리스트"는 2004년 이후 데이터 
<br> 이 둘을 비교했을 때 약 20개 영화가 빠지게 됨.(왕의남자, 괴물 등 당연히 들어가야하는 영화인데 빠져있어서 
<br> "서울시 데이터"를 추가적으로 받아 
<br> 스크린수와 전국 관객수를 다시 확인후 새로운 "movie_resized2.csv" 가 필요함
<br> 
<br>
#### TO DO
<br>  새롭게 받은 "movie_resized2.csv" 파일로 
<br>  감독,배급사 중복값 다시 정리 
<br>  연도, 계절 열 추가
<br>  ...
<br>  크롤링 완료하기
<br>  딥러닝 optimizer 생각해보기
<br> 

---

### 2023.03.20.월 회의록   

<데이터 확인 및 전처리 과정>   
(DataFrame 이름을 df로 하는 것을 가정)   
1. df.isnull().sum() : 결측치 확인하기   
2. df.drop(['컬럼명1', '컬럼명2', ...], axis=1, inplace=True) : 불필요한 '컬럼' 삭제하기   
3. df['컬럼명1'].fillna('기타', inplace=True) : null값 기타로 변경하기   
4. df.drop(df.loc[df['컬럼명1'].isnull()].index, inplace=True) : null 값이 있는 '행' 삭제하기   
5. 컬럼별로 unique 값 확인하기   
```
category_cols = ['영화명','감독', '배급사', '영화유형','영화형태','국적','장르','등급','영화구분',]   
for col in category_cols:   
    print('컬럼 [{}] UNIQUE : {}'.format(col, len(df[col].unique())))   
```
6. 감독 -> 대표 1명으로 줄이기   
7. 등급 -> 4개 등급으로 줄이기   
```
for idx, row in df.iterrows():   
    if ',' in row['등급']:   
        ratings = row['등급'].split(',')   
        # 전체, 12, 15, 청소년관람불가 순    
        if '전체' in ratings:   
            df['등급'][idx] = '전체관람가'   
        elif '12' in ratings:   
            df['등급'][idx] = '12세관람가'   
        elif '15' in ratings:   
            df['등급'][idx] = '15세관람가'   
        else:   
            df['등급'][idx] = '청소년관람불가'   
    elif df['등급'][idx] == '18세관람가' or df['등급'][idx] == '제한상영가':    
        df['등급'][idx] = '청소년관람불가'   
    else:   
        df['등급'][idx] = df['등급'][idx].replace('이상', '')   
```
8. 데이터값이 숫자여야 하는데 dtype이 object로 나오는 컬럼을 숫자값으로 변경하기   
```
# 쉼표','를 없애줘야 int로 변환이 가능   
df['전국 매출액'] = df['전국 매출액'].str.replace(pat=r',', repl=r'', regex=True)   
df['전국 매출액'] = df['전국 매출액'].astype('int64')   
   
# '전국스크린수', '전국 매출액', '전국 관객수' 하나씩 해주기   
```
9. 수치형 데이터 상관관계 확인   

<전처리 정해야할 것>   
* 개봉연월일 -> '연' / '월' 데이터로 컬럼 나눴으면
* 개봉연도 2020~2022 -> '코로나여부' 컬럼을 생성해서 1, 0 체크해서 쓸 수 있으면 어떨까
* 국적 -> 한국, 북미, 아시아, 유럽 등 그룹으로 묶어주기
* 배급사 -> 동일한 배급사 명칭 하나로
* '영화 흥행 여부'? target 컬럼 만들기
<br>
<br>
